...
class ObsidianNoteFormatter:
    ...
    def __init__(self, use_time_prefix: bool = False):
        """Initialize formatter.

        Args:
            use_time_prefix: Whether to prefix filenames with time for sorting
        """
        self.use_time_prefix = use_time_prefix

    def format_event(
        self, event: Dict[str, Any], tags: Optional[List[str]] = None
    ) -> str:
    ...
    def generate_filename(self, event: Dict[str, Any]) -> str:
        ...
        if self.use_time_prefix:
            # Use time prefix for ordering within day folders
            start_time = event["start_time"]

            # All-day events get 0000 prefix to sort first
            if event.get("is_all_day", False):
                time_prefix = "0000"
            else:
                time_prefix = start_time.strftime("%H%M")

            return f"{time_prefix} - {safe_subject}.md"
        else:
            # Use date prefix for flat structure
            start_time = event["start_time"]
    ...
    def _build_content(self, event: Dict[str, Any]) -> str:
        ...
        if event.get("is_all_day", False):
            ...
            if "end_time" in event:
                ...
                content_parts.append(
                    f"**Time**: {start_str} - {end_str} {timezone_str}"
                )
            else:
            ...
            for attendee in event["attendees"]:
                ...
                if isinstance(attendee, dict):
                    if "emailAddress" in attendee:
                        # Graph API structure has emailAddress sub-object
