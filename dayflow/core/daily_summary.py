...
class DailySummaryGenerator:
    ...
    def generate_daily_summary(self, summary_date: date, events: List[Dict[str, Any]]) -> Optional[Path]:
        ...
        content = self._format_daily_summary(summary_date, sorted_events)
    ...
    def _format_daily_summary(self, summary_date: date, events: List[Dict[str, Any]]) -> str:
        ...
        if all_day_events:
            ...
            for event in all_day_events:
                ...
                cancelled = ' ~~(Cancelled)~~' if event.get('is_cancelled', False) else ''
        ...
        if timed_events:
            ...
            for event in timed_events:
                ...
                if event.get('attendees'):
                    attendee_names = [a.get('name', a.get('email', 'Unknown')) for a in event['attendees'][:5]]
                    attendee_list = ', '.join([f'[[{name}]]' for name in attendee_names])
    ...
    def update_daily_summaries(self, events_by_date: Dict[date, List[Dict[str, Any]]]) -> Dict[str, int]:
