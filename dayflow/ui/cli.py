...
def get_token_info():
    ...
    try:
        ...
        return {
            ...
            'age_minutes': int((now - datetime.fromisoformat(token_data.get('acquired_at', ''))).total_seconds() / 60)
        }
    except Exception:
        return None
...
def auth_status():
    ...
    if not token_info['valid']:
        ...
        if hours_left > 2:
            click.echo(f"Expires in {hours_left:.1f} hours ({token_info['expires_at'].strftime('%Y-%m-%d %H:%M:%S')})")
        else:
...
@click.option('--no-interactive', is_flag=True, hidden=True, help='Non-interactive mode for testing')
def auth_login(no_interactive):
    ...
    # Validate token
    if len(token) < 100:
        click.echo("Invalid token (too short). Please make sure you copied the access token.")
        return
...
@click.option('--interval', type=int, default=5, help='Sync interval in minutes (for continuous mode)')
@click.option('--no-daily-summary', is_flag=True, help='Skip creating daily summary notes')
def sync(start, end, continuous, interval, no_daily_summary):
    ...
    from dayflow.core.sync import CalendarSyncEngine, ContinuousSyncManager
    from dayflow.core.graph_client import GraphAPIError
    from dayflow.vault import VaultConfig, VaultConnection, VaultConfigError
    ...
    engine = CalendarSyncEngine(access_token, vault_connection, create_daily_summaries=not no_daily_summary)
            ...
            if result:
                ...
                if vault_connection:
                    ...
                    if daily_created > 0 or daily_updated > 0:
                        click.echo("")
                        if daily_created > 0:
                            click.echo(f"📅 Created {daily_created} daily summary note{'s' if daily_created != 1 else ''}")
                        if daily_updated > 0:
                            click.echo(f"📅 Updated {daily_updated} daily summary note{'s' if daily_updated != 1 else ''}")
                else:
...
@click.option('--link-meeting/--no-link-meeting', default=True, help='Link to current meeting')
@click.option('--editor/--no-editor', '-e/-E', default=False, help='Open in editor for full content')
@click.option('--template', '-T', type=click.Choice(['meeting', 'idea', 'task', 'reference']), help='Use a note template')
def note(title, link_meeting, editor, template):
        ...
        try:
            ...
            if meeting_path and meeting_path.exists():
                ...
                if current:
                    ...
                    upcoming = matcher.find_upcoming_meeting(meeting_path, lookahead_minutes=5)
                    if upcoming:
    ...
    folder_type = 'daily_notes' if config.get_location('daily_notes') else 'calendar_events'
...
@click.option('--template', type=click.Choice(['para', 'gtd', 'time_based', 'zettelkasten']), 
...
def vault_init(path, template, interactive):
    ...
    if not (vault_path / '.obsidian').exists():
        if not click.confirm(f"'{path}' doesn't appear to be an Obsidian vault. Continue anyway?"):
            raise click.Abort()
...
@click.argument('template', type=click.Choice(['para', 'gtd', 'time_based', 'zettelkasten']))
...
def status():
    ...
    try:
        ...
        if meeting_path and meeting_path.exists():
            ...
            if upcoming:
                mins_until = (upcoming['start_time'] - datetime.now(timezone.utc)).total_seconds() / 60
                click.echo(f"🔜 Next: {upcoming['title']} (in {int(mins_until)} minutes)")
