...
class TestSyncVaultIntegration:
    ...
    def test_sync_creates_notes_in_vault(self, mock_graph_client, vault_connection, mock_graph_events):
        ...
        assert result['daily_summaries_created'] > 0  # Should create at least one daily summary
    ...
    @patch('dayflow.core.sync.GraphAPIClient')
    def test_sync_updates_existing_notes(self, mock_graph_client, vault_connection, mock_graph_events):
    ...
    @patch('dayflow.core.sync.GraphAPIClient')
    def test_sync_with_date_range(self, mock_graph_client, vault_connection, mock_graph_events):
    ...
    def test_sync_error_handling(self, mock_graph_client, vault_connection):
        ...
        mock_client_instance = mock_graph_client.return_value
        mock_client_instance.fetch_calendar_events.side_effect = Exception("Network error")
    ...
    @patch('dayflow.core.sync.GraphAPIClient')
    def test_sync_without_daily_summaries(self, mock_graph_client, vault_connection, mock_graph_events):
        ...
        engine = CalendarSyncEngine('fake_token', vault_connection, create_daily_summaries=False)
