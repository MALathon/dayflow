...
class TestHTMLCleanupIntegration:
    ...
    def test_complex_teams_meeting_full_flow(self):
        ...
        assert (
            "- **Project Updates**: Review progress on current initiatives"
            in normalized["body"]
        )
        assert "- **Blockers**: Discuss any issues or impediments" in normalized["body"]
        assert (
            "- **Planning**: Look ahead to next week's priorities" in normalized["body"]
        )
        ...
        assert (
            ...
            == "https://teams.microsoft.com/l/meetup-join/19%3ameeting_NjM0ZTI4MjktMzQ1Yi00MTRmLWE5NDMtYzg5OTZkZDFiNGY0%40thread.v2/0?context=%7b%22Tid%22%3a%2272f988bf-86f1-41af-91ab-2d7cd011db47%22%2c%22Oid%22%3a%22f4d7b952-84d5-4643-9e5e-4fc98d86e9f0%22%7d"
        )
